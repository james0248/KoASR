...
def prepare_dataset_with_json(org_train_data, org_val_data, args, processor):
    ...
    json_train_data = org_train_data.map(read_json, batched = True, batch_size = 1, remove_columns = org_train_data.column_names)
    json_val_data = org_val_data.map(read_json, batched = True, batch_size = 1, remove_columns = org_val_data.column_names)
...
def clean_text(text: str) -> Union[str, None]:
    ...
    text = re.sub(r'\(.+\)\/\(.+\)',lambda match: match.group(0).split('/')[0][1:-1], text)
    text = re.sub(r'-|~',' ',text)
...
def read_wav(batch):
    data, sampling_rate = librosa.load(batch["path"], offset=batch["start"], duration=batch["length"], sr=None)
...
def predict_dialect(file, model, processor, device):
    ...
    data_list = []
    for start in starts:
        data, sampling_rate = librosa.load(file, offset = start, duration = min(length_per_batch,total_length-start), sr=None)
